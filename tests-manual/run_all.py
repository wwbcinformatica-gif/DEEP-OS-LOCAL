...
TESTES = [
    ...
    ("test_tools_headless.py", "Ferramentas por ambiente: VPS (sem tela) x PC (com desktop)",
     "TODOS OS TESTES PASSARAM"),
    ("test_instancias_isolamento.py", "Instancias: isoladas por assinante (tenant do JWT, nao da query)",
     "TODOS OS TESTES PASSARAM"),
    ("test_gpu_toggle.py", "GPU do Ollama/llama.cpp: o botao das configuracoes salva de verdade",
     "TODOS OS TESTES PASSARAM"),
    ("test_charon_ambiente.py", "Charon: sabe se esta no PC (com tela) ou na VPS (headless)",
     "TODOS OS TESTES PASSARAM"),
    ("test_identidade_dois_assistentes.py", "Identidade: Charon e Jarvis leem do tenant (nao do config global)",
     "TODOS OS TESTES PASSARAM"),
