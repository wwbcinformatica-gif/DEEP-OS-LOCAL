...
def _is_safe_path(target: Path) -> bool:
    ...
    Quando full_access estiver ativo ou file_controller=allow, permite qualquer caminho.
    Caso contrario, restringe a pastas seguras (home do usuario).
    """
    try:
        # Import local para evitar circular import na inicializacao
        from core.permissions import is_full_access, get_category_perm
        if is_full_access():
            return True
        if get_category_perm("file_controller") == "allow":
            return True
        if get_category_perm("file_controller") == "deny":
            return False
    except Exception:
        pass
...
def _resolve_path(raw: str) -> Path:
    ...
    # Normalizar letra de drive isolada (ex: 'G:' -> 'G:\\') para acessar a raiz da unidade
    if re.match(r'^[a-zA-Z]:$', raw.strip()):
        raw = raw.strip() + '\\'
...
def list_files(path: str = "desktop", show_hidden: bool = False, tts_friendly: bool = False) -> str:
    try:
        ...
        if tts_friendly:
            # Resumido e sem emojis/tamanhos para TTS do Charon.
            # Sempre lista TODAS as pastas primeiro; limita apenas a quantidade de arquivos.
            max_files = 100
            total = len(items)
            folders = [i for i in items if i.endswith('/')]
            files = [i for i in items if not i.endswith('/')]
            shown_files = files[:max_files]
            lines = [f"Pasta {target.name}: {total} itens ({len(folders)} pastas)."]
            if folders:
                lines.append("Pastas: " + ", ".join(f.replace('/', '') for f in folders))
            if shown_files:
                clean_files = [f.split(' (')[0] for f in shown_files]
                lines.append("Arquivos: " + ", ".join(clean_files))
                if len(files) > max_files:
                    lines.append(f"E mais {len(files) - max_files} arquivos.")
            elif not folders:
                return f"Directory is empty: {target.name}/"
            return " ".join(lines)
...
# Extensoes reconhecidas como visiveis/abriveis
_OPENABLE_EXTS = {
    ".txt", ".md", ".log", ".xml", ".html", ".htm", ".css", ".js", ".json",
    ".py", ".ts", ".doc", ".docx", ".pdf", ".xls", ".xlsx", ".ppt", ".pptx",
    ".csv", ".odt", ".ods", ".odp",
    ".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp", ".svg", ".ico", ".tiff",
    ".mp3", ".wav", ".mp4", ".avi", ".mkv", ".mov", ".wmv", ".flac", ".ogg",
    ".zip", ".rar", ".7z", ".tar", ".gz",
}


def open_file(path: str = "desktop", name: str = "") -> str:
    """Abre um arquivo com o aplicativo padrao do sistema."""
    try:
        base   = _resolve_path(path)
        target = (base / name) if name else base
        if not _is_safe_path(target):
        ...
        # Verifica se a extensao e abrivel
        ext = target.suffix.lower()
        if ext and ext not in _OPENABLE_EXTS:
            # Extensoes desconhecidas tambem podem ser abertas pelo app padrao,
            # mas avisamos para evitar surpresas
            print(f"[file_controller] Opening with default app anyway: {ext}")

        print(f"[file_controller] Opening: {target}")
...
def file_controller(
    ...
    try:
        if action == "list":
            # file_controller e usado principalmente pela voz (Charon);
            # retorna versao resumida e amigavel para TTS
            return list_files(path, tts_friendly=True)
        ...
        elif action in ("open", "open_file", "abrir"):
