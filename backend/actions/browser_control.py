...
def _close_native_tabs(all_tabs: bool = False) -> str:
    """
    Fecha abas no navegador REAL do usuario (que foi aberto via _open_native).
    Usa atalhos de teclado: Ctrl+W fecha a aba ativa; Ctrl+F4 fecha a aba ativa (alternativa).
    Para fechar todas as abas, usa Alt+F4 ou fecha a janela diretamente.
    """
    try:
        import pyautogui
        import time as _t
        ...
        if all_tabs:
            # Para fechar todas as abas, usar Alt+F4 (fecha a janela inteira)
            # ou Ctrl+Shift+W que pode nao funcionar em todos os navegadores
            if platform.system() == "Darwin":
                # macOS: Command+Shift+W fecha a janela
                pyautogui.hotkey("command", "shift", "w")
            else:
                # Windows: Alt+F4 fecha a janela (mais confiavel que Ctrl+Shift+W)
                pyautogui.hotkey("alt", "f4")
            _t.sleep(0.5)
            return "Closed browser window (Alt+F4)."
        ...
        return f"Could not close browser tabs: {e}"
...
class _BrowserSession:
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / time.strftime("jarvis_browser_%Y%m%d_%H%M%S.png"))
...
def browser_control(
    ...
    if action == "close_all":
        if _registry.has_active_sessions():
            result = _registry.close_all()
        else:
            result = _close_native_tabs(all_tabs=True)
        _log(player, result)
        return result
    ...
    try:
        ...
        if action == "click":
            ...
            if _registry.has_active_sessions():
                ...
                result = _close_native_tabs(all_tabs=False)
