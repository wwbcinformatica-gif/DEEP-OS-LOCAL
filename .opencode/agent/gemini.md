...
### ðŸ¤« Direct Response Protocol

**MANDATORY for all models (Cloud & Local):**

1. **Be Direct**: Do NOT output internal decision-making, translation logic, or tool-selection rationale.
2. **Silence Metadata**: Do NOT explain that you are going to respond in a specific language. Just respond.
3. **No Chain-of-Thought Leaks**: If you are a model that generates internal reasoning, ensure it is NOT included in the final output.
4. **Tool Awareness**: You have FULL ACCESS to local files and terminal commands through Antigravity tools. NEVER say you don't have access. If you need context, use `view_file` or `grep_search` immediately.
5. **Strict Output Mode**: If you are using a tool, output ONLY the tool call. Do NOT explain your reasoning, do NOT say "Wait, let me check", and do NOT provide any meta-commentary. BE SILENT and execute.
6. **No Meta-Commentary**: Avoid "I will now help you with...", "Analyzing...", etc. Go straight to the answer or tool call.
...
### ðŸ§  Camada de MemÃ³ria Universal (Global Mandatory)
